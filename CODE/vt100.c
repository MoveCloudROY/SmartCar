...
/**
 * @description: Clear screen & delete all lines saved in the scrollback buffer
 * & move cursor to (0,0)
 * @param void
 * @return: void
 */
...
/**
 * @description: Draw a horizontal line beginning at (row, col) with length of
 * (len)
 * @param row
 * @param col
 * @param len
...
/**
 * @description: Draw a vertical line beginning at (row, col) with length of
 * (len)
 * @param row
 * @param col
 * @param len
...
/**
 * @description: Fill a box with upper left corner at (row, col) with n_rows and
 * n_cols
 * @param s_row
 * @param s_col
 * @param n_row
 ...
 * @return: void
 */
void vt_fill_box(uint16_t s_row,
                 uint16_t s_col,
                 uint16_t n_rows,
                 uint16_t n_cols,
                 char ch) {
...
/**
 * @description: Draw a framed box with upper left corner at (row, col) with
 * n_rows and n_cols
 ...
 * @return: void
 */
void vt_draw_box(uint16_t s_row,
                 uint16_t s_col,
                 uint16_t n_rows,
                 uint16_t n_cols,
                 char h_fill,
 ...
 * @return: void
 */
void vt_draw_bitmap(uint16_t s_row,
                    uint16_t s_col,
                    uint16_t n_rows,
                    uint16_t n_cols,
                    const uint8_t* bitmap,
                    vt_back_color color_on,
                    vt_back_color color_off) {
  uint16_t row = 0;
  uint16_t col = 0;
  for (row = s_row; row < (s_row + n_rows); row++) {
    vt_move_to(row, s_col);
...
void vt_draw_rgb888_cwh(uint8_t* buffer, uint16_t n_rows, uint16_t n_cols) {
  ...
  for (row = 0; row < n_rows; row++) {
    for (col = 0; col < n_cols; col++) {
      ...
      PRINTF("\x1b[48;2;%d;%d;%dm\x1b[38;2;%d;%d;%dm◾\x1b[39m\x1b[49m", r, g, b,
             r, g, b);
    }
    PRINTF("\n");
  }
